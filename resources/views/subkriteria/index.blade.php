                    ...
                    <div class="col-md-4">
                        ...
                        <select id="filterKriteria" class="form-control">
                            ...
                            @foreach ($kriterias as $krit)
                                <option value="{{ $krit->nama }}">{{ $krit->nama }}</option>
                            @endforeach
                        </select>
                        ...
                        @foreach ($data->groupBy('kriteria.nama') as $namaKriteria => $items)
                            @foreach ($items as $index => $d)
                                <tr class="row-kriteria" data-kriteria="{{ $namaKriteria }}">
                                    ...
                                    <td>
                                        <span
                                            class="badge badge-{{ $d->kriteria->tipe == 'benefit' ? 'success' : 'danger' }}">
                                    ...
                                    <td>
                                        ...
                                        <form method="POST" action="{{ route('subkriteria.destroy', $d->id) }}"
                                            class="d-inline" onsubmit="return confirm('Yakin hapus sub kriteria ini?')">
                                            @csrf
                                            @method('DELETE')
